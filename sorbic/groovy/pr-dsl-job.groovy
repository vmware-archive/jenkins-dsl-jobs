...
def github_json_data = slurper.parseText(build.getEnvironment()['GITHUB_JSON_DATA'])
...
github_json_data['open_prs'].each { pr ->
    folder {
        name("sorbic/pr/${pr.number}")
        displayName("PR #${pr.number}")
        description = "${pr.title}\n\n${pr.body}"
    }

    try {
        existing_job = Jenkins.instance.getJob('sorbic').getJob('pr').getJob("${pr.number}").getJob('main-build')
        ...
        // PR Main Job
        job(type: BuildFlow) {
            name = "sorbic/pr/${pr.number}/main-build"
            ...
            script_template = template_engine.createTemplate(
                readFileFromWorkspace('jenkins-master-seed', 'templates/pr-main-build-flow.groovy')
            )
        ...
        def pr_clone_job = job {
            name = "sorbic/pr/${pr.number}/clone"
                ...
                job_properties.appendNode(
                    'hudson.plugins.copyartifact.CopyArtifactPermissionProperty').appendNode(
                        'projectNameList').appendNode(
                            'string').setValue("sorbic/pr/${pr.number}/*")
                github_project_property = job_properties.appendNode(
                    'com.coravy.hudson.plugins.github.GithubProjectProperty')
                github_project_property.appendNode('projectUrl').setValue("https://github.com/${github_repo}")
            }
        ...
        job {
            ...
            steps {
                ...
                copyArtifacts("sorbic/pr/${pr.number}/clone", 'workspace.cpio.xz') {
        ...
        // PR Unit Tests
        job {
            name = "sorbic/pr/${pr.number}/tests"
            displayName("Tests")
            concurrentBuild(allowConcurrentBuild = true)
            description("${pr.title}\n\n${pr.body}")
            label('worker')
