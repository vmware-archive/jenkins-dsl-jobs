...
def master_main_job = job(type: BuildFlow) {
    ...
    buildFlow(
        readFileFromWorkspace('jenkins-master-seed', 'libnacl/groovy/master-main-build-flow.groovy')
    )
...
def master_unit_job = job {
    ...
    steps {
        ...
        // Run Unit Tests
        shell(readFileFromWorkspace('jenkins-master-seed', 'libnacl/scripts/run-unit.sh'))
    }
...
dsl_job = job {
    ...
    steps {
        dsl {
            removeAction('DELETE')
            text(
                readFileFromWorkspace('jenkins-master-seed', 'libnacl/groovy/pr-dsl-job.groovy')
            )
        }
    }
