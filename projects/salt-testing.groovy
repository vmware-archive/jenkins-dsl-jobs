// Salt-Testing Jenkins jobs seed script
import groovy.json.*
import groovy.text.*
...
def project = new JsonSlurper().parseText(build.getEnvironment().SEED_PROJECTS)['salt-testing']
...
// Main develop branch job
buildFlowJob("${project.name}/develop-main-build") {
    displayName('Develop Branch Main Build')
    description(project.description)
    label('worker')
    ...
    buildFlow(
        readFileFromWorkspace('maintenance/jenkins-seed', 'projects/salt-testing/groovy/develop-main-build-flow.groovy')
    )
...
// Clone Develop Job
freeStyleJob("${project.name}/develop/clone") {
        ...
        job_properties.appendNode(
            'hudson.plugins.copyartifact.CopyArtifactPermissionProperty').appendNode(
                'projectNameList').appendNode(
                    'string').setValue('salt-testing/develop/*')
        github_project_property = job_properties.appendNode(
            'com.coravy.hudson.plugins.github.GithubProjectProperty')
        github_project_property.appendNode('projectUrl').setValue("https://github.com/${project.repo}")
    ...
    template_context = [
        ...
        sudo_salt_call: true,
        virtualenv_name: 'salt-testing-develop',
...
// Lint Develop Job
freeStyleJob("${project.name}/develop/lint") {
    displayName('Lint')
    concurrentBuild(allowConcurrentBuild = true)
    ...
    template_context = [
        ...
        sudo_salt_call: true,
        virtualenv_name: 'salt-testing-develop',
        virtualenv_setup_state_name: 'projects.salt-testing.lint'
    ]
    script_template = template_engine.createTemplate(
        readFileFromWorkspace('maintenance/jenkins-seed', 'common/templates/branches-envvars-commit-status.groovy')
    )
    ...
    steps {
        ...
        copyArtifacts("${project.name}/develop/clone", 'workspace.cpio.xz') {
            buildNumber('${CLONE_BUILD_ID}')
        }
        shell(readFileFromWorkspace('maintenance/jenkins-seed', 'common/scripts/decompress-workspace.sh'))

        // Run Lint Code
        shell(readFileFromWorkspace('maintenance/jenkins-seed', 'projects/salt-testing/scripts/run-lint.sh'))
    }
...
freeStyleJob("${project.name}/pr/jenkins-seed") {
    ...
    steps {
        dsl {
            ...
            text(
                readFileFromWorkspace('maintenance/jenkins-seed', 'projects/salt-testing/groovy/pr-dsl-job.groovy')
            )
        }
    }
